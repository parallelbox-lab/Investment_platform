<?php 
    ...
    if(isset($_REQUEST['email'])){
        ...
        $check = mysqli_query($conn,"SELECT * FROM users WHERE email = '$email' and password = '$password' and role_id !='0' ") or die(mysqli_error($conn));
        $check_array = mysqli_fetch_array($check);
        if($check_array){
            $message = '<div class="alert alert-danger">User exist! Check details and try again</div>';
            
            
            $_SESSION['admin_id'] = $check_array['id'];
            $_SESSION['surname'] = $check_array['surname'];
            
            if($check_array['role_id']=="1"){
